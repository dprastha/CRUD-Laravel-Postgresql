...
<div class="card">
   ...
   <div class="card-body card-block">
      <form action="{{ route('suratkeluar.update', $item->id) }}" method="POST">
         @method('PUT')
         @csrf
         {{-- Id --}}
         ...
         {{-- Pengirim --}}
         <div class="form-group">
            <label for="penerima" class="form-control-label">Penerima</label>
            <input type="text" 
            name="penerima" 
            id="penerima" 
            value="{{ old('penerima') ? old('penerima') : $item->penerima }}" class="form-control @error('penerima') is-invalid @enderror">
            @error('penerima') <div class="text-muted">{{ $message }}</div> @enderror
         </div>
         {{-- Tanggal --}}
         <div class="form-group">
